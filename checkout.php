...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['full_name'])) {
    ...
    $payment_method = $_POST['payment_method'] ?? 'bank_transfer'; // Default to bank_transfer
    ...
    if (!empty(trim($_POST['full_name']))) {
        ...
        try {
            ...
            // --- NEW: Group items by seller to calculate subtotals ---
            ...
            foreach ($cart_items as $product_id => $quantity) {
                ...
                if (!isset($seller_notifications[$seller_id])) {
                    $seller_notifications[$seller_id] = ['subtotal' => 0, 'items' => []];
                }
                $seller_notifications[$seller_id]['subtotal'] += $product['price'] * $quantity;
                $seller_notifications[$seller_id]['items'][] = "- {$product['name']} (จำนวน: {$quantity})";
            }

            $sql_seller_detail = "INSERT INTO order_seller_details (order_id, seller_id, subtotal, status) VALUES (?, ?, ?, ?)";
            $stmt_seller_detail = $conn->prepare($sql_seller_detail);

            foreach ($seller_notifications as $seller_id => &$notification) {
                $seller_status = ($payment_method === 'cod') ? 'cod' : 'pending_payment';

                // Insert per-seller details
                $stmt_seller_detail->bind_param("iids", $order_id, $seller_id, $notification['subtotal'], $seller_status);
                $stmt_seller_detail->execute();

                // Prepare and send LINE message
                $stmt_uid = $conn->prepare("SELECT line_user_id FROM users WHERE id = ?");
                $stmt_uid->bind_param("i", $seller_id);
                $stmt_uid->execute();
                $user_result = $stmt_uid->get_result()->fetch_assoc(); // fetch once
                $stmt_uid->close();

                if ($user_result && !empty($user_result['line_user_id'])) {
                    $line_user_id = $user_result['line_user_id'];
                    $item_list_string = implode("\n", $notification['items']);
                    $formatted_subtotal = number_format($notification['subtotal'], 2);

                    if ($payment_method === 'cod') {
                        $message = "📦 มีคำสั่งซื้อแบบเก็บเงินปลายทาง! (Order #{$order_id})\n\n";
                    } else {
                        $message = "🔔 มีคำสั่งซื้อใหม่! (Order #{$order_id})\n\n";
                    }

                    $message .= "รายการสินค้า:\n{$item_list_string}\n\n";
                    $message .= "ยอดรวม (ร้านของคุณ): {$formatted_subtotal} บาท\n";

                    if ($payment_method === 'cod') {
                        $message .= "กรุณาเตรียมจัดส่งสินค้าและตรวจสอบในระบบ";
                    } else {
                        $message .= "กรุณารอผู้ซื้อชำระเงินและตรวจสอบในระบบ";
                    }

                    sendLinePushMessage($conn, $line_user_id, $message);
                }
            }
